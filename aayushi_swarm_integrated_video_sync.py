...
while True:
    # === Retrieve telemetry ===
    self_drone_id = telem_data.get('self_drone_id')
    ...
    msg_att = telem_data.get("self_yaw")
    
    if msg_att:
        
        last_yaw = f"{float((msg_att).decode('utf-8')) * (180.0 / 3.14159):.2f}"
        
    # print(f"Self drone id: {self_drone_id} GPS: {last_alt} {last_lat} {last_lon} Yaw: {last_yaw}")
